    ...
    def ui(self, is_img2img):
        with gr.Group():
            ...
            with contextlib.suppress(AttributeError):
                if is_img2img:
                    fetch_tags.click(fn=fetch, inputs=[image, post_id], outputs=[self.boxxIMG])
                else:
                    fetch_tags.click(fn=fetch, inputs=[image, post_id], outputs=[self.boxx])

        return [image, post_id, fetch_tags]
